...
\begin{lstlisting}[language=c, caption={Das Programm dim.c zum Dimmen von einer LED mithilfe eines Joysticks in Y-Richtung}, captionpos=b]
#include<stdio.h>
#include<stdint.h>
...
int main(void){
    ...
    int adc_value = 0; // ADC value variable
    ...
    while (1)
    {
        ...
        if(!(GPIO_PORTM_DATA_R &= (1<<1))){
            ...
            TIMER2_TAMATCHR_R = 8000 + (2000 - adc_value) * 4 * 0.95; // Set the match value for PWM from 95\% to 5\%
        }
        else{
            // If the button is pressed
            minmax = !minmax;
            TIMER2_TAMATCHR_R = 8000 + (2 - (minmax ? 0x04 : 0x00)) * 4000 * 0.95; // Set the match value for PWM for either min or max
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{normal_better.png}
...
\begin{figure}[h]
  ...
  \begin{subfigure}{.45\textwidth}
    \centering
    \includegraphics[width=\linewidth]{brightest.png}
    \caption{Oszillogramm zur hellsten Einstellung}
  \end{subfigure}
  \begin{subfigure}{.45\textwidth}
    \centering
    \includegraphics[width=\linewidth]{darkest.png}
